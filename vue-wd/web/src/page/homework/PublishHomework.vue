<template>
  <div class="homework_wrap warpper">
    <div class="header">
      <span class="impot_title">发布作业</span>
    </div>
    <div class="content_center">
      ...
      <el-row :gutter="10" style="padding:5px 0;">
        ...
        <el-col :span="20" style="line-height:40px;">
          <el-select v-model="classes" size=30 placeholder="请选择班级" multiple @change="initData">
            <el-option v-for="item in options" :key="item.value" :label="item.label" :value="item.value"></el-option>
          </el-select>
          <span style="color:red;font-size:14px;" v-if="userInfo.schoolModel !== 2">（支持给多个班级发布作业）</span>
        </el-col>
      </el-row>
      <el-row :gutter="10" style="padding:5px 0;" v-if="classes.length > 0">
        ...
        <el-col :span="21" style="line-height:40px;">
          <el-button type="success" size="mini" @click="setClassBtn">设置</el-button><span style="font-size:14px;">（可设置<i style="font-style: normal;color:red;">部分同学接收</i>及<i style="font-style: normal;color:red;">是否在线提交）</i></span>
        </el-col>
      </el-row>
      <el-row :gutter="10" style="padding:5px 0;">
        <el-col :span="3" style="line-height:40px;">
        ...
        <el-col :span="4" style="line-height:40px;">
          <el-select v-model="lessonId" size=30 placeholder="请选择课程">
      ...
      <el-row :gutter="10" style="padding:5px 0;">
        ...
        <el-col :span="10">
          <el-input type="textarea" :autosize="{ minRows: 4, maxRows: 6}" placeholder="请输入内容（最多输入500字）" :maxlength="500" resize="none" v-model="workContent"></el-input>
        </el-col>
      </el-row>
      <el-row :gutter="10" style="padding:5px 0;">
        <el-col :span="3" style="line-height:40px;">
    ...
    <div class="btn_wrap">
      <el-button type="success" @click="clearData">清空</el-button>
      <el-button type="success" @click="publishData" :loading="loading">发布</el-button>
    </div>
    <transition name="el-fade-in-linear">
          ...
          <div style="padding:10px 0 0 15px;">
            <div style="height:550px;overflow-y:auto;">
              <el-row>
                ...
                <el-col :span="21">
                  <el-radio style="margin:5px 10px 0 0;vertical-align: middle;" @change="changeClass" v-model="selectClass" :label="item.value" border v-for="(item, index) in selectOption" :key="index">{{item.label}}</el-radio>
                </el-col>
              </el-row>
              <div v-for="(item, index) in studentList" :key="index" v-if="selectClass == item.classId">
    ...
    methods: {
      ...
      changeClass () {
        for (let i = 0; i < this.studentList.length; i++) {
          if (this.selectClass === this.studentList[i].classId && this.studentList[i].student === null) {
            this.getStudentList(this.selectClass, (data) => {
              for (let k = 0; k < data.length; k++) {
                data[k].isRequire = false
                data[k].isReceive = true
              }
              for (let j = 0; j < this.studentList.length; j++) {
                if (parseInt(this.studentList[j].classId) === parseInt(this.selectClass)) {
                  this.studentList[j].student = data
                }
              }
            })
          }
      ...
      setClassBtn () {
        ...
        this.selectClass = this.selectOption[0].value
        if (this.studentList[0].student === null) {
          this.getStudentList(this.selectClass, (data) => {
            for (let k = 0; k < data.length; k++) {
              data[k].isRequire = false
              data[k].isReceive = true
      ...
      uploadFile (e) {
        ...
        type !== 'msword' && type !== 'xls' &&
        type !== 'xlsx' && type !== 'vnd.openxmlformats-officedocument.wordprocessingml.document' &&
        type !== 'vnd.openxmlformats-officedocument.spreadsheetml.sheet') {
          if (file.type.split('/')[0] === 'video') {
            toKen = 'SCHOOL_WORK_VIDEO'
            this.uploadReader(file, 'video', toKen, name)
          } else {
      ...
      uploadPicture (e) {
        if (this.uploadImg.length >=  9) {
          this.$message({message: '最多上传9张照片', type: 'warning'})
          return
        }
        let file = e.target.files[0]
        let type = file.type.split('/')[1]
      ...
      publishData () {
        this.workDate = this.tools.formateTime(new Date().getTime(), 'YYYY-MM-DD HH:mm')
        if (!this.endDate) {
          this.$message.error('请选择截止提交时间')
          return
        }
        if (this.workContent || this.uploadImg.length > 0 || this.workFile.length > 0 || this.workVideo.length > 0) {
        ...
        this.loading = true
        api.teacherAddWork({schoolId: this.$route.params.schoolId, classes: dataList, lessonId: this.lessonId,
        workDate: this.workDate, endDate: this.tools.formateTime(this.endDate, 'YYYY-MM-DD HH:mm'),
        workContent: this.workContent, workImg: imgs, workFile: JSON.stringify(this.workFile), workVideo: this.workVideo}, (data) => {
          this.loading = false
          this.$alert('发布成功！', '提示', {

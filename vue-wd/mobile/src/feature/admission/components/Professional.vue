<template>
  <div v-transfer-dom>
    <popup v-model="isShowPopup" height="90vh">
      <div>
        <group title="点击按钮添加招生专业">
          <cell v-for="(item, index) in tempInfoList" :key="index" :title="item" @click.native="onButtonClick(index)" is-link></cell>
        </group>
        <x-button style="margin: 10px auto;width: 90%;display:block;margin-bottom:70px" text="增加招生专业" type="primary" @click.native="addItem"></x-button>
        <!--添加弹出框-->
        <popup v-model="showPopupInput" is-transparent>
          <div flots="default" style="width: 95%;background-color:#fbf9fe;margin:0 auto;border-radius:5px;padding-top:10px;">
            <p v-if="showContentLengthHit" style="fontSize:12px;marginLeft:15px;color:red;">选项不能超过10个字</p>
            <div>
...
export default {
  ...
  watch: {
    isShow(newValue, old) {
      ...
      if (!this.tools.isNull(this.defaultData)) {
  ...
  methods: {
    ...
    addItem () {
      if (this.tempInfoList.length >= 10) {
    ...
    saveEditTextValue () {
      ...
      if (this.tools.getLength(this.newEditTextValue) > 20) {
      ...
      if (this.isModify) {
        // 修改
        this.isModify = false
        this.tempInfoList.splice(this.currentIndex, 1, this.newEditTextValue)
      } else {
        this.tempInfoList.push(this.newEditTextValue)
      }
      this.newEditTextValue = null
      this.showPopupInput = false
    },
    actionSheetClick (key) {
      switch (key) {
        case 'modify':
          this.showPopupInput = true
          this.newEditTextValue = this.tempInfoList[this.currentIndex]
          this.isModify = true
          break
        case 'delete':

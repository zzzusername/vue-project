<template>
  <div>
    <group title="星级班级评定" label-width="7em" label-margin-right="0.5em" label-align="left">
      <selector title="目标年级" v-if="classInfo.length > 1" v-model="gradeId" @on-change="chooseGrade" :options="optionGrade" placeholder="请选择年级"></selector>
      <selector title="目标班级" v-if="gradeId" v-model="classId" :options="optionClass" @on-change="chooseClass('true')" placeholder="请选择班级"></selector>
      <cell title="目标班级" value="请选择班级" @click.native="chooseClass('false')" v-else is-link value-align="left"></cell>
      <cell title="班级评定项" is-link @click.native="showEvaluate = true" value-align="left" :value="classEvaluateText"></cell>
      <cell title="班级扣分数" v-if="checkedClass" :value="itemStore" value-align="left"></cell>
      <cell title="关联违规学生" is-link :value="studentNum" value-align="left" @click.native="getStudent"></cell>
      <cell title="学生扣分评定项" v-if="studentIds.length > 0" :value="studentEvaluateText" value-align="left" is-link @click.native="showStudentEvaluate"></cell>
      <cell title="学生扣分数" v-if="studentIds.length > 0" :value="studentItemStore" value-align="left"></cell>
      <x-textarea title="评定描述" :rows="5" placeholder="请选输入评定描述" v-model="evaluateDesc"></x-textarea>
      <media-upload :imageData="contentImgPathList" :args="mediaArgs" :imageType="mediaImageType" :callBack="mediaCallBack"></media-upload>
    </group>
    <!-- 班级评定 -->
    <popup v-model="showEvaluate" height="90%">
      <div :style="{'position': 'relative', 'height': scrollerHeight + 'px'}">
        <scroller ref="manage_scroller">
          <group v-if="classEvaluateList.length > 0" gutter="-1px" v-for="(item, index) in classEvaluateList" :key="index">
            <cell :title="item.projectName" value-align="left" is-link :border-intent="false" :arrow-direction="item.showUp ? 'up' : 'down'" @click.native="openSecond(item)">
              <!-- <i slot="default" @click.self="shouHelp(item.projectDesc)" style="color: #999;float: right;margin-right: 20px;" class="iconfont icon-bangzhu"></i> -->
            </cell>
            <template v-if="item.showUp">
              <checklist :options="item.evaluate" style="color:#999;" v-model="checkedClass" :max="1"></checklist>
            </template>
          </group>
        </scroller>
        <p v-if="classEvaluateList.length <= 0" style="padding: 15px;text-align: center;color: #999;font-size: 16px;">暂无学生评定选项，请登陆<span style="user-select: all !important;color: rgb(46, 132, 212);">https://edu.qunxiaozhu.com/</span>进行设置</p>
      </div>
      <div style="display:flex;position: fixed;width: 100vw;bottom: 0;height: 50px;">
        <div style="flex: 1;background: rgb(230, 230, 230)" @click="showEvaluate = false">
      ...
      <div :style="{'position': 'relative', 'height': scrollerHeight + 'px'}">
        <scroller ref="manage_scroller">
          <group v-if="studentEvaluateList.length > 0" gutter="-1px" v-for="(item, index) in studentEvaluateList" :key="index">
            ...
            <template v-if="item.showUp">
              <checklist :options="item.evaluate" style="color:#999;" v-model="checkedStudent" :max="1"></checklist>
            </template>
          </group>
        </scroller>
    ...
    <alert v-model="showDesc" title="简介">{{projectDesc}}</alert>
    <detail-tabbar :actionLink="submit" :actions="actions"></detail-tabbar>
  </div>
</template>

<script>
  import { Group, Cell, Popup, Radio, Alert, Selector, XTextarea } from 'vux'
  import DetailTabbar from '@/components/DetailTabbar.vue'
  import MediaUpload from '@/components/MediaUpload.vue'
  import Checklist from '@/components/checklist/Index.vue'
  import api from '../api'
  import studentApi from '../../education/api'
  export default {
    components: {Checklist, Group, Cell, Popup, Radio, Alert, Selector, XTextarea, DetailTabbar, MediaUpload},
    data () {
      return {
        scrollerHeight: 0,
        classInfo: [],
    ...
    mounted () {
      ...
      api.classMoralTreeList({schoolId: this.$route.params.schoolId}, (data) => {
    ...
    methods: {
      submit () {
        ...
        if (!this.checkedClass) {
          this.tools.warnToast('请选择班级评定项')
          return
        }
        api.evaluateClass({schoolId: this.$route.params.schoolId, classId: this.classId, itemId: this.checkedClass, evaluateDesc: this.evaluateDesc, evaluateImg: this.contentImgPathList, stuItemId: this.checkedStudent, studentIds: this.studentIds}, (data) => {
      ...
      getStudent () {
        ...
        studentApi.queryStudentListOrderName({schoolId: this.$route.params.schoolId, classId: this.classId}, (data) => {
          this.studentList = []
      ...
      showStudentEvaluate () {
        api.studentMoralTreeList({schoolId: this.$route.params.schoolId, projectType: 2}, (data) => {
          data.forEach(element => {
            element.showUp = false
            element.evaluate = []
            for (let i = 0; i < element.items.length; i++) {
      ...
      confirmClass () {
        let smallClassEvaluate = []
        for (let i = 0; i < this.classEvaluateList.length; i++) {
          for (let k = 0; k < this.classEvaluateList[i].items.length; k++) {
            let item = {
              key: this.classEvaluateList[i].items[k].itemId,
              value: this.classEvaluateList[i].items[k].itemName,
              store: this.classEvaluateList[i].items[k].itemStore
            }
            smallClassEvaluate.push(item)
          }
        }
        smallClassEvaluate.forEach(element => {
          if (parseInt(element.key) === parseInt(this.checkedClass)) {
            this.classEvaluateText = element.value
            this.itemStore = element.store + '分'
          }
        })

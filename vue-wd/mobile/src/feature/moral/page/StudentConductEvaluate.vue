<template>
  <div>
    <group title="学生操行评定" label-width="7em" label-margin-right="0.5em" label-align="left">
      ...
      <cell :title="type == 'plus' ? '加分数' : '减分数'" :value="studentItemStore" value-align="left"></cell>
...
<script>
  ...
  export default {
    ...
    mounted () {
      ...
      api.moralTeacherClass({schoolId: this.$route.params.schoolId, isHaveLessonClass: true}, (data) => {
        this.classInfo = data
    ...
    methods: {
      ...
      saveEvaluate () {
        ...
        api.evaluateStudent({schoolId: this.$route.params.schoolId, studentIds: this.studentIds, itemId: this.checkedStudent, evaluateDesc: this.evaluateDesc, evaluateImg: this.contentImgPathList, type: 'teacher'}, (data) => {
